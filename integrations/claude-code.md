...
2. Voice: librarian (`styles/librarian.md`). No exclamation points, no em dashes.
3. Check, then render PDF with OpenBook tools only (never browser `file://` print):
...
Acknowledge in one short reply that you have read the OpenBook paths and will check + render via OpenBook scripts before handoff.
...
**Order:** Cover → Sixty-second → Narrative (chapters) → What's next (≤5 plain questions, pen room) → What we learned → Appendix.
